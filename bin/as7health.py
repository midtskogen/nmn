...
class ErrorCatalog:
    ...
    _catalog = {
        ...
        "NMN_HOST_UNREACHABLE_80": {"type": "warning", "description": "Cannot connect to norskmeteornettverk.no on port 80 (HTTP).", "reason": "Connectivity to the main NMN web server is required for some services or indicates a general network issue.", "fix": "Check this station's internet connection, firewall rules, and DNS settings. Verify the host is online."},
        "NMN_HOST_UNREACHABLE_22": {"type": "warning", "description": "Cannot connect to norskmeteornettverk.no on port 22 (SSH).", "reason": "Connectivity to the NMN SSH server is critical for the autossh reverse tunnel to function.", "fix": "Check this station's internet connection, firewall rules (especially outbound port 22), and DNS settings."},
        "NMN_AMS_MAILBOX_CLEAN_FAILED": {"type": "warning", "description": "Cleaning old messages from '/var/spool/mail/ams' failed.", "reason": "The mailbox cleanup script returned an error.", "fix": "Check the script output and run 'sudo python3 /home/meteor/nmn/bin/clean_mailbox.py /var/spool/mail/ams --days 14' manually."},
        "NMN_AMS_MAILBOX_CLEAN_TIMEOUT": {"type": "warning", "description": "Cleaning old messages from '/var/spool/mail/ams' timed out.", "reason": "The mailbox is very large and cleanup took too long.", "fix": "Run the cleanup manually or increase the timeout."}
    }
...
class AS7Diagnostic:
    ...
    def check_nmn_connectivity(self):
        ...
        if any(self._check_port_open(host, 80, timeout=10) for _ in range(3)):
            self.log_success(f"Successfully connected to {host} on port 80 (HTTP).")
        else:
            self.log_issue("NMN_HOST_UNREACHABLE_80")

        if any(self._check_port_open(host, 22, timeout=10) for _ in range(3)):
    ...
    def check_nmn_python_imports(self):
        ...
        for pkg in sorted(list(imports_to_check)):
            ...
            try:
                ...
                subprocess.run(cmd_import, check=True, capture_output=True, text=True, timeout=120, cwd=self.meteor_pwd.pw_dir)
