...
CONFIG = {
    ...
    "IMAGE_MODEL_CONFIGS": {
        'resnet50': 'meteor_resnet50_model_quantized.pth', # Changed from .pth
        'efficientnet_b0': 'meteor_efficientnet_b0_model.pth',
    },
    "VIDEO_MODEL_CONFIGS": {
        'r2plus1d_18': 'meteor_r2plus1d_18_quantized.pth',
    },
...
def quantize_model(model_name, original_model_path, quantized_model_path, args, data_dir, layers_to_quantize):
    """
    Applies quantization to a trained model to reduce its size.
    """
    device = "cpu" # Quantization is a CPU-based operation
    logging.info(f"--- Starting Quantization for {model_name} ---")
    
    # 1. Load the original, trained model
    model = load_model_helper(model_name, original_model_path, args)
    model.to(device)
    # model.eval() is called inside load_model_helper, which is causing the issue.

    # 2. Prepare the model for quantization
    # --- FIX: Set the model to training mode before preparing ---
    model.train() 
    model.qconfig = torch.quantization.get_default_qat_qconfig('qnnpack')
    
    # The DeprecationWarning is expected and can be ignored for now.
    model_prepared = torch.quantization.prepare_qat(model)
    
    # 3. Calibrate the model with sample data
    logging.info("Calibrating model with sample data...")
    model_prepared.eval() # Set to eval mode for calibration
    train_loader, _, _ = get_dataloaders(args, data_dir, split=True)
    with torch.no_grad():
        for i, (inputs, _) in enumerate(tqdm(train_loader, desc="Calibrating")):
    ...
    logging.info(f"Saved quantized model to '{quantized_model_path}'")
...
def load_model_helper(model_name: str, model_file: str, args: argparse.Namespace) -> nn.Module:
    """Loads a model's state dict, supporting various architectures including quantized models."""
    device = "cpu" # Quantized models run on the CPU
    ...
    # Instantiate the base model structure
    model = get_model(model_name, args, params)
    
    # If loading a quantized model, prepare the model structure for quantization first
    if 'quantized' in model_file:
        logging.info(f"Preparing model '{model_name}' for loading quantized state_dict.")
        # --- FIX: Set the model to training mode before preparing ---
        model.train() 
        model.qconfig = torch.quantization.get_default_qat_qconfig('qnnpack')
        # We don't need to calibrate, just prepare and convert an empty shell
        model_prepared = torch.quantization.prepare_qat(model)
        model = torch.quantization.convert(model_prepared)

    model.to(device)
    # Load the state dictionary into the correctly structured model
...
def _get_stacking_data(args, image_model_configs, video_model_configs, data_dir, class_to_idx):
    ...
    device = "cpu"
    logging.info(f"--- Generating stacking data on CPU to ensure compatibility with quantized models ---")
    ...
    # --- FIX: Load the ORIGINAL full-precision models and quantize them dynamically in memory ---
    
    # Get Image Model Predictions
    for model_name, quantized_model_path in image_model_configs.items():
        original_model_path = quantized_model_path.replace("_quantized", "_model")
        if not os.path.exists(original_model_path):
            original_model_path = quantized_model_path.replace("_quantized", "")
        ...
        with torch.no_grad():
            for img_batch, _, _ in tqdm(loader, desc=f"Predicting with {model_name}"):
                preds = quantized_model(img_batch.to(device)).cpu().numpy().flatten()
    ...
    for model_name, quantized_model_path in video_model_configs.items():
        original_model_path = quantized_model_path.replace("_quantized", "_model")
        if not os.path.exists(original_model_path):
            original_model_path = quantized_model_path.replace("_quantized", "")
        ...
        model = load_model_helper(model_name, original_model_path, args)
        model.to(device)
        quantized_model = torch.quantization.quantize_dynamic(model, {torch.nn.Linear, torch.nn.Conv3d}, dtype=torch.qint8)
        
        preds_list = []
        with torch.no_grad():
            for _, vid_batch, _ in tqdm(loader, desc=f"Predicting with {model_name}"):
                preds = quantized_model(vid_batch.to(device)).cpu().numpy().flatten()
                prob_positive = 1.0 - preds if class_to_idx.get('meteor') == 0 else preds
                preds_list.extend(prob_positive)
        all_predictions[f"pred_{model_name}"] = preds_list
        del model, quantized_model
...
def build_ensemble_mode(args):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # --- Train and Quantize Image Models ---
        # Handle ResNet50 (Train -> Quantize)
        model_name = 'resnet50'
        original_img_model_path = 'meteor_resnet50_model.pth'
        quantized_img_model_path = CONFIG["IMAGE_MODEL_CONFIGS"][model_name]
        if args.resume and os.path.exists(quantized_img_model_path):
            logging.info(f"Resuming: Found existing quantized model '{quantized_img_model_path}'. Skipping.")
        else:
            img_args = argparse.Namespace(**vars(args), input_type='image', model_name=model_name)
            _run_training(img_args, data_dir, original_img_model_path, params_file=None)
            quantize_model(model_name, original_img_model_path, quantized_img_model_path, img_args, data_dir, 
                           layers_to_quantize=[torch.nn.Linear, torch.nn.Conv2d])

        # Handle other image models (just train)
        for model_name, model_path in CONFIG["IMAGE_MODEL_CONFIGS"].items():
            if model_name == 'resnet50': continue # Already handled
        ...
        # --- Train and Quantize Video Models ---
        model_name = 'r2plus1d_18'
        original_vid_model_path = 'meteor_r2plus1d_18_model.pth'
        quantized_vid_model_path = CONFIG["VIDEO_MODEL_CONFIGS"][model_name]
        if args.resume and os.path.exists(quantized_vid_model_path):
            logging.info(f"Resuming: Found existing quantized model '{quantized_vid_model_path}'. Skipping.")
        else:
            vid_args = argparse.Namespace(**vars(args), input_type='video', model_name=model_name)
            _run_training(vid_args, data_dir, original_vid_model_path, params_file=None)
            quantize_model(model_name, original_vid_model_path, quantized_vid_model_path, vid_args, data_dir, 
                           layers_to_quantize=[torch.nn.Linear, torch.nn.Conv3d])
