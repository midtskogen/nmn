...
for ip in $(cat /etc/cameras.ip); do
    ...
    echo convert -pointsize 12 $(~/bin/brightstar.py -f 2.5 $(~/bin/timestamp /tmp/image-$$.jpg) $dir/lens.pto 2> /dev/null | sed 's/[(),]//g;s/'\''//g' | awk '{x=$1; y=$2; az=$3; alt=$4; $1=$2=$3=$4=""; sub(/ */, ""); gsub(/[^-A-Za-z0-9 .]/, "", $0); printf("-stroke white -fill none -draw \"circle %f,%f %f,%f\" -stroke none -fill white -annotate +%f+%f \"%s [%.2f %.2f]\"\n", x, y, x+7, y, x+11, y-4, $0, az, alt)}') /tmp/empty-$$.png /tmp/labels-$$.png | bash
