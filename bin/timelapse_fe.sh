...
ffmpeg -f concat -safe 0 -r 30 -i /tmp/frames_fe.txt -crf 22 -c:v libx264 -preset veryfast -pix_fmt yuv420p -y "$DIR/timelapse_new.mp4" \
...
if [ -s /tmp/frames_fe_hires.txt ]; then
    ffmpeg -f concat -safe 0 -r 30 -i /tmp/frames_fe_hires.txt -crf 25 -c:v libx264 -preset veryfast -pix_fmt yuv420p -y "$DIR/timelapse_hires_new.mp4" \
