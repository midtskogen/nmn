    ...
    def process(self):
        ...
        try:
            ...
            if not base_media_path or not os.path.exists(base_media_path):
                self.errors.append(f"Kunne ikke hente kildefil for kamera {self.cam} kl {self.time_utc.strftime('%H:%M')}.")
                return
            ...
            self.errors.append(f"Feil for kamera {self.cam} kl {self.time_utc.strftime('%H:%M')}")
...
def download_for_single_station(task_id, station_id, json_payload_str):
    ...
    for i, item in enumerate(files_to_process):
        update_status(station_status_file, "progress", {"step": i, "total": len(files_to_process), "message": f"Behandler fil {i+1}/{len(files_to_process)}...", "files": results, "errors": errors})
    ...
    for i, job in enumerate(blending_jobs):
        update_status(station_status_file, "progress", {"step": len(files_to_process)-len(blending_jobs)+i, "total": len(files_to_process), "message": f"Venter på {len(blending_jobs)-i} sporfletting(er)...", "files": results, "errors": errors})
        ...
        else: errors.append(f"Kunne ikke brenne inn spor for {os.path.basename(job['overlay_filepath'])}")
        ...
        for cam_num, images in images_by_camera.items():
            if len(images) > 1:
                ...
                if stack_images([img['path'] for img in images], output_filepath, task_id):
                    ...
                    errors.append(f"Kunne ikke lage stablet bilde for kamera {cam_num}.")
...
def main_download_coordinator(master_task_id, json_payload, user_ip):
    ...
    try:
        ...
        if not station_ids: raise ValueError("Ingen stasjoner valgt.")
        if len(station_ids) > MAX_STATIONS_PER_REQUEST: raise ValueError(f"For mange stasjoner valgt (maks {MAX_STATIONS_PER_REQUEST}).")
        
        with open(STATIONS_FILE, 'r') as f: valid_stations = json.load(f)
        for sid in station_ids:
            if sid not in valid_stations: raise ValueError(f"Ugyldig stasjons-ID: {sid}")
        
        file_type = data.get('file_type', 'lowres')
        if file_type not in FILE_TYPE_LIMITS: raise ValueError(f"Ugyldig filtype: {file_type}")
        ...
        if num_files > limit: raise ValueError(f"{num_files} filer er valgt, som er for mange (maks {limit}).")
        
        if not active_pass_data:
            if not (1 <= int(data.get('length', 0)) <= MAX_SEQUENCE_LENGTH): raise ValueError(f"Ugyldig 'Antall' (1-{MAX_SEQUENCE_LENGTH}).")
            if not (1 <= int(data.get('interval', 0)) <= MAX_SEQUENCE_INTERVAL): raise ValueError(f"Ugyldig 'Intervall' (1-{MAX_SEQUENCE_INTERVAL}).")
        ...
        for station_id in station_ids:
            ...
            if station_info.get("station", {}).get("quota"):
                ...
                if site_usage_bytes + estimated_request_size > per_site_quota_bytes:
                    aggregated_errors.append(f"Din personlige datakvote ({PER_SITE_QUOTA_LIMIT_MB}MB) for {station_code} er brukt opp for i dag.")
                    continue
                if total_usage_bytes + estimated_request_size > total_quota_bytes:
                    aggregated_errors.append(f"Den totale daglige datakvoten ({TOTAL_QUOTA_LIMIT_MB}MB) for {station_code} er brukt opp.")
                    continue
    ...
    try:
        ...
        while time.time() - start_time < 1800: # 30 min timeout
            ...
            update_status(status_file, "progress", {"step": percentage_done, "total": 100, "message": f"Behandler filer ({int(total_steps_done)}/{int(total_steps_overall)})..."})
        ...
        for task_id, task_info in sub_tasks.items():
            ...
            if os.path.exists(s_file):
                with open(s_file, 'r') as f: s_data = json.load(f)
                station_code = all_stations_data.get(task_info['station_id'], {}).get('station', {}).get('code', 'UKJENT')
...
def main():
    ...
    if action in actions:
        ...
        print("""
<!DOCTYPE html><html lang="no"><head><title>Tilgang til videoer og bilder</title>
...
<div class="container">
    <header><h1>Tilgang til videoer og bilder</h1></header>
    <main class="main-content">
        <div id="map-panel">
            <h2>1. Velg stasjoner på kartet</h2>
            <div id="map"></div>
            <p class="map-description">
                Arkivet dekker som regel noen uker bakover i tid, men det kan være stor variasjon mellom stasjonene, og noen stasjoner kan også være ute av drift. Noen stasjoner har treg forbindelse og nedlasting kan ta lang tid. Intervallfunksjonen sammen med et passende antall kan være nyttig til å sjekke skydekke eller nordlys over en periode. Lyndata fra Meteorologisk institutt.
            </p>
        </div>
        <div id="control-panel">
            <h2>2. Velg tidspunkt og filtype</h2>
            <form id="download-form">
                <div class="form-group">
                    <h6>Valgte stasjoner:</h6>
                    <div id="station-list-container">
                        <p style="color: #6c757d; margin: 0;">Ingen stasjon valgt</p>
                        <ul id="station-list" style="display: none;"></ul>
                    </div>
                </div>
                <div class="form-group">
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="cloud-toggle"> Vis skydekke (oppdatert daglig)</label></div>
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="aurora-toggle"> Vis nattlys</label></div>
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="terminator-toggle"> Vis nattemørke</label></div>
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="satellite-toggle"> Vis satelittpasseringer</label></div>
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="aircraft-toggle"> Vis flytrafikk</label></div>
                    <div class="toggle-wrapper">
                        <label class="checkbox-label-inline"><input type="checkbox" id="lightning-toggle"> Vis lynnedslag (siste 7 dager)</label>
                        <label class="checkbox-label-inline" id="lightning-filter-label" style="display: none; margin-left: 15px;">
                            <input type="checkbox" id="lightning-24h-toggle"> bare siste 24 timer
                        </label>
                    </div>
                    <div class="toggle-wrapper"><label class="checkbox-label-inline"><input type="checkbox" id="meteor-toggle"> Vis meteorer (siste 7 dager)</label></div>
                </div>
                <div class="form-group date-group-flex">
                   <label for="date-display">Dato (UTC):</label>
                   <div class="date-input-wrapper">
                        <input type="text" id="date-display" placeholder="YYYY-MM-DD" readonly>
                        <input type="date" id="date" name="date" required>
                   </div>
                   <button type="button" id="last-night-btn" title="Velg siste natt for den først valgte stasjonen">Siste natt</button>
                   <button type="button" id="now-button" title="Sett tiden til to minutter siden">Nå</button>
               </div>
                <div class="form-group time-group"><div><label for="hour">Time (0-23 UTC):</label><select id="hour" name="hour" required><option value="" disabled selected>--</option></select></div>
                <div><label for="minute">Minutt (0-59):</label><select id="minute" name="minute" required><option value="" disabled selected>--</option></select></div></div>
                <div class="form-group time-group">
                    <div><label for="length">Antall:</label><select id="length" name="length" required><option value="" disabled selected>--</option></select></div>
                    <div><label for="interval">Intervall (minutt):</label><select id="interval" name="interval" required><option value="" disabled selected>--</option></select></div>
                </div>
                ...
                <fieldset class="form-group combined-file-type">
                     <div class="primary-type-group">
                         <label><input type="radio" name="primary_file_type" value="video"> Video</label>
                         <label><input type="radio" name="primary_file_type" value="image" checked> Bilde</label>
                    </div>
                     <div class="options-group" id="file-options-group">
                         <label class="checkbox-label-inline"><input type="checkbox" id="high-resolution-switch"> Høg oppløsning</label>
                         <label class="checkbox-label-inline" id="long-integration-label"><input type="checkbox" id="long-integration-switch">Lang integrering</label>
                    </div>
                </fieldset>
                <div class="button-group">
                    <button type="submit" id="download-button">Start nedlasting</button>
                    <button type="button" id="cancel-button" style="display: none;">Avbryt</button>
                </div>
...
<div class="full-width-container" id="satellite-panel-container"><div class="container" id="satellite-panel"><h2>Nylige satellittpasseringer</h2><div id="satellite-list"><p style="color: #6c757d; margin: 0;">Laster passeringer...</p></div></div></div>
<div class="full-width-container" id="aircraft-panel-container"><div class="container" id="aircraft-panel"><h2>Flytrafikk (siste 24 timer)</h2><div id="aircraft-list"><p style="color: #6c757d; margin: 0;">Laster flytrafikk...</p></div></div></div>
<div class="full-width-container" id="lightning-panel-container"><div class="container" id="lightning-panel"><h2>Lynnedslag (siste 7 dager)</h2><div id="lightning-list"><p style="color: #6c757d; margin: 0;">Laster lynnedslag...</p></div></div></div>
...
<div class="container">
    <footer id="results-panel">
        <h2>Resultat</h2>
        <div id="progress-container" style="display: none;"><p>Status: <span id="progress-text">starter...</span></p>
