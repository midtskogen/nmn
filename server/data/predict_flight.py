...
def get_opensky_token(task_id, client_id, client_secret):
    ...
    if os.path.exists(TOKEN_CACHE_FILE):
        try:
            ...
            # Use cached token if it's not close to expiring.
...
def update_and_get_flight_database(task_id, access_token):
    ...
    # Prune flights that were last seen more than 24 hours ago.
...
def get_flight_track(task_id, icao24, time_within_flight, access_token):
    """Fetches the full trajectory (track) for a specific flight, using a local cache."""
    ...
    for i in range(len(path) - 1):
        ...
        if time_diff > max_interval_sec:
            ...
            for j in range(1, int(num_segments)):
                ...
                interp_point[0] = p1[0] + fraction * time_diff # Time
                interp_point[1] = p1[1] + fraction * (p2[1] - p1[1]) # Latitude
                interp_point[2] = p1[2] + fraction * (p2[2] - p1[2]) # Longitude
                if p1[3] is not None and p2[3] is not None:
                    interp_point[3] = p1[3] + fraction * (p2[3] - p1[3]) # Baro Altitude
                else:
                    interp_point[3] = p1[3] or p2[3]
                if len(p1) > 9 and len(p2) > 9: # Check if geo altitude is present
...
def fetch_and_process_track(args):
    """
    Worker function for the process pool. Takes a flight, fetches its full track,
    ...
    if not track_data or not track_data.get('path'): return None
    ...
    for station_id, station_info in stations_data.items():
        ...
        # Iterate through each point in the flight's trajectory.
        for point in interpolated_path:
            p_time, p_lat, p_lon, baro_alt = point[0], point[1], point[2], point[3]
            geo_alt = point[9] if len(point) > 9 else None
            # Prioritize geometric altitude over barometric if available.
            p_alt = geo_alt if geo_alt is not None else baro_alt

            if not all([p_lat, p_lon, p_alt]): continue
            
            # Convert aircraft's lat/lon/alt to ECEF coordinates.
            ...
            # Convert ENU to astronomical Azimuth/Altitude.
            az_deg, alt_deg = enu_to_az_alt(e, n, u)
        
            if alt_deg > 10: # Check if the aircraft is above the horizon.
    ...
    # Filter out isolated points to ensure we only get continuous crossings.
    ...
    # Construct the final pass data structure.
    ground_track = []
    seen_coords = set()
    ...
    # Group consecutive visible points into single "camera_view" events.
...
def find_all_crossings(task_id):
    ...
    try:
        ...
        # Pre-filter flights to those with valid origin/destination data.
        ...
        # Create a shortlist of candidate flights whose great-circle path passes near a station.
        ...
        # Limit the number of flights to process in a single run.
        ...
        # Use a ProcessPoolExecutor for parallel processing of flight tracks.
        ...
        # Dynamically calculate the time window covered by the found passes.
