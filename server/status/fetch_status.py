...
def sync_camera_contacts():
    """Merge contact_email entries from camera_contacts.json into cameras.json.

    cameras.json is normally regenerated by calfetch.py, but that script is not
    currently run on a schedule. This keeps the contact emails in cameras.json
    in sync with the manually maintained camera_contacts.json so that status
    notifications reach the right recipients.
    """
    contacts_path = CAMERAS_JSON.with_name(CAMERA_CONTACTS_FILENAME)
    try:
    ...
    updated = 0
    for sid, c_info in contacts.items():
        if sid in cameras and isinstance(c_info, dict):
            contact_email = c_info.get('contact_email')
            if contact_email is not None:
                cameras[sid].setdefault('station', {})['contact_email'] = contact_email
                updated += 1
    ...
    if updated:
        try:
            ...
            print(f"  [Contacts] Warning: could not write {CAMERAS_JSON}: {e}")
            return 0
    return updated


def get_contact_emails(station_id):
    """Get contact_email list, preferring camera_contacts.json over cameras.json."""
    contacts_path = CAMERAS_JSON.with_name(CAMERA_CONTACTS_FILENAME)
    try:
        with open(contacts_path, 'r', encoding='utf-8') as f:
            contacts = json.load(f)
        contact_email = contacts.get(station_id, {}).get('contact_email')
        if contact_email is not None:
...
def main():
    ...
    contacts_updated = sync_camera_contacts()
    print(f"Loaded {station_count} stations from {CAMERAS_JSON}")
    if contacts_updated:
        print(f"Updated contact emails for {contacts_updated} stations from {CAMERA_CONTACTS_FILENAME}")
