...
int main(int argc, char **argv) {
  ...
  res = __sunriset__(year, month, day, lon, lat, alt, 0, &start, &end);

  switch (res) {
    case 0: printf("%02d:%02d %02d:%02d\n", (int)end, (int)((end - (int)end) * 60.0), (int)start, (int)((start - (int)start) * 60)); break;
    case +1: printf("Never below\n"); break;
    case -1: printf("Never above\n"); break;
  }
  return 0;
}
...
/******************************************************/
{
  ...
  *lon = v + w;                     /* True solar longitude */
  if (*lon >= 360.0) *lon -= 360.0; /* Make it 0..360 degrees */
}
